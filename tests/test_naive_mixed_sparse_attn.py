...
def test_full_coverage_no_vertical(
    ...
    out = naive_mixed_sparse_attn_fwd(
...
def test_partial_last_block(batch_size, seqlen, head_size, dtype):
    ...
    k = torch.randn(B, N, H, D, dtype=dtype)
    v = torch.randn(B, N, H, D, dtype=dtype)
    # Non-causal: vertical columns cover remaining positions without causal restriction
    # so the result should match full non-causal attention
...
def test_vs_cuda():
    ...
    torch.testing.assert_close(out_naive, out_cuda, atol=2e-2, rtol=1e-2)
